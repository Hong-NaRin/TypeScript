...
let arr2 = func([1, 2, 3] as [number, number, number]); // 튜플로 추론하는 방법 1 -> 타입 단언을 통해 튜플 타입으로 추론
let arr3 = func<[number, number, number]>([1, 2, 3]); // 방법 2 -> [number, number, number]가 <T>에 할당되고 튜플 타입으로 반환
